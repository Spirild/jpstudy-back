...
func (ts *ThirdService) SparkDemo(question string) (string, error) {
	...
	//握手并建立websocket 连接
	conn, resp, err := d.Dial(assembleAuthUrl1(ts.sparkUrl, ts.apiKey, ts.apiSecret), nil)
	if err != nil {
		ts.Log.Error(err.Error(), zap.Any("rsp", readResp(resp)))
		return "", err
	...
	go func() {
		...
		data := genParams1(ts.appid, question)
		conn.WriteJSON(data)
	...
	for {
		...
		if code != 0 {
			ts.Log.Info("Spark Status Error", zap.Any("payload", data["payload"]))
			return "", errSparkCode
		}
		...
		if status != 2 {
			...
			ts.Log.Info(fmt.Sprintf("total_tokens:%f", totalTokens))
			conn.Close()
			break
		}
...
var (
	errSparkCode = errors.New("spark status error")
)
